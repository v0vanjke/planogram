...
const rotate = (direction) => {
  ...
  apiStore.shopBoxes[boxID].rotation.z = Number(
    (anglePI + step * direction).toFixed(5)
  );
};
...
<template>
  <v-card
    ...
    width="200"
    :style="`position: absolute; top: ${position.y - 298}px; left: ${
      position.x + 20
    }px`"
  >
    <v-card-text class="d-flex flex-column">
      ...
      <v-text-field
        class="my-5"
        v-model="apiStore.shopBoxes[boxID].index"
        type="number"
      ...
      <div class="d-flex justify-space-between">
        ...
        <div
          class="text-center font-weight-thin mr-n2 mt-1"
          style="font-size: 36px"
        >
          {{
            Math.round(
              (360 / Math.PI / 2) * apiStore.shopBoxes[boxID].rotation.z
            )
          }}&deg;
        </div>
        <v-btn @click="rotate(-1)" icon size="x-small" variant="outlined">
      ...
      <v-btn @click="copyBox" text variant="outlined" class="mt-4">
        копировать
      </v-btn>
    </v-card-text>
  </v-card>
