...
<template>
  ...
  >
    ...
    <div v-if="isSelected" >
      ...
      <div v-for="i in [1, -1]" :key="i">
        <Plane
          :width=10000
          :height=0.2
          :position="{z: - (box.position.z + computedZ / 2) + 0.1, y: i * (computedY / 2 + 0.1)}"
        >
        ...
        <Plane
          ...
          :position="{z: - (box.position.z + computedZ / 2) + 0.1, x: i * (computedX / 2 + 0.1)}"
        >
          <LambertMaterial :color="shadowBorderColor()"/>
        </Plane>
      </div>
    ...
    <Circle
      ...
      :segments=20
      :rotation="{z: Math.PI * 2 - box.rotation.z + appStore.cameraAzimuth}"
      :position="{ x: -computedX / 2 + 1.8 , y: computedY / 2 - 1.4, z: computedZ / 2 + 0.05}"
    >
