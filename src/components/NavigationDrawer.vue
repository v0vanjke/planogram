...
const listVendor = computed(() => {
    ...
    return apiStore.apiCollections.map(item => item.vendor).filter((value, index, self) => self.indexOf(value) === index)
})
...
const listCategory = computed(() => {
    if (!apiStore.apiCollections) return []
    // from list of objects return list of unique item
    return apiStore.apiCollections.map(item => item.category).filter((value, index, self) => self.indexOf(value) === index)
})
...
<template>
    <div class=" px-3 rounded-xl pt-8 mr-8 mb-8 navigation">
        ...
        <div>
            <VBtn color="grey" @click="appStore.changeMode('roomPlanner')" block class="mb-1"> Строительство </VBtn>
            <VBtn color="grey" @click="appStore.changeMode('goodsPlanner')" block> Планирование </VBtn>
        </div>
        ...
        <div color="transparent" v-if="appStore.mode === 'roomPlanner' && apiStore.apiBoxes">
            ...
            <div v-for="box, boxID in apiStore.apiBoxes" :key="boxID">
                <VCard flat @click="apiStore.addShopBox({ ...box, position: { x: -300, y: 0, z: 0 } })" class="my-2">
                    <VCardSubtitle>
                        <div>{{ box._id }}</div>
                    </VCardSubtitle>
                    <VCardText>
        ...
        <div color="transparent" v-if="appStore.mode === 'goodsPlanner'">
            <v-card flat class="mt-5" color="transparent" v-if="appStore.mode === 'goodsPlanner'">
                ...
                <v-select class="mt-n6" label="производители" hide-selected density="comfortable" theme="dark"
                    variant="solo" v-model="appStore.selectedCollectionVendor" :items="listVendor"></v-select>
            </v-card>
