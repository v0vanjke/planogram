...
<template>
    <!-- pointer -->
    <Renderer @mousedown="appStore.eventMouseDown" @mouseup="appStore.eventMouseUp" ref="renderer" antialias resize pointer
        shadow :orbit-ctrl="appStore.orbCtrlSettings">
        ...
        <Scene ref="scene" background="hsl(0, 0%, 60%)">
            ...
            <SpotLight cast-shadow color="hsl(0, 0%, 100%)" :intensity="0.2" :position="{ x: 0, y: 300, z: 300 }"
                :angle="Math.PI / 8" :penumbra="0.1" :shadow-map-size="{ width: 2048, height: 2048 }" />
            <SpotLight cast-shadow color="hsl(0, 0%, 100%)" :intensity="0.2" :position="{ x: 0, y: 300, z: -300 }"
                :angle="Math.PI / 8" :penumbra="0.1" :shadow-map-size="{ width: 2048, height: 2048 }" />

            <SpotLight cast-shadow color="hsl(0, 0%, 100%)" :intensity="0.5" :position="{ x: 500, y: 300, z: 0 }"
                :angle="Math.PI / 8" :penumbra="0.1" :shadow-map-size="{ width: 2048, height: 2048 }" />
            <SpotLight cast-shadow color="hsl(0, 0%, 100%)" :intensity="0.5" :position="{ x: -500, y: 300, z: 0 }"
                :angle="Math.PI / 8" :penumbra="0.1" :shadow-map-size="{ width: 2048, height: 2048 }" />

            <Box :width="500" :height="300" :depth="1" :position="{ y: -0.9 }" :rotation="{ x: -Math.PI / 2 }">
                <LambertMaterial color="hsl(0, 0%, 5%)" />
            </Box>
            <Box :width="500" :height="300" :depth="2" :position="{ y: -2.4 }" :rotation="{ x: -Math.PI / 2 }">
                <LambertMaterial color="hsl(0, 0%, 15%)" />
            </Box>
            <Box :width="500" :height="300" :depth="4" :position="{ y: -5.4 }" :rotation="{ x: -Math.PI / 2 }">
                <LambertMaterial color="hsl(25, 50%, 20%)" />
            </Box>
            <Box :width="500" :height="300" :depth="12" :position="{ y: -13.4 }" :rotation="{ x: -Math.PI / 2 }">
                <LambertMaterial color="hsl(40, 100%, 30%)" />
            </Box>

            <Plane :width="500" :height="300" receive-shadow :rotation="{ x: -Math.PI / 2 }">
                <Collection v-for="v, collectionID in apiStore.shopCollections" :key="collectionID" :id="collectionID" />
    ...
    <v-card class="helperCard" v-if="false">
        overBoxID:
        {{ appStore.overBoxID }}
        <br>
        overCollectionID:
